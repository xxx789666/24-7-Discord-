...
只輸出 3 個優先項目，不要其他說明。${safetyRules}`;
  ...
  const perspective1 = await geminiGenerate(prompt1);
...
只輸出 3 個優先項目，不要其他說明。${safetyRules}`;
  ...
  const perspective2 = await geminiGenerate(prompt2);
...
只輸出 3 個優先項目，不要其他說明。${safetyRules}`;
  ...
  const perspective3 = await geminiGenerate(prompt3);
...
語氣專業但親切，全部繁體中文。${safetyRules}`;
  ...
  const synthesis = await geminiGenerate(prompt4);
